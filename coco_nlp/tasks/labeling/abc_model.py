...
import coco_nlp
from coco_nlp.embeddings import ABCEmbedding, BareEmbedding
from coco_nlp.generators import ABCGenerator, CorpusGenerator, BatchDataSet
from coco_nlp.layers import KConditionalRandomField
from coco_nlp.logger import logger
from coco_nlp.metrics.sequence_labeling import get_entities
from coco_nlp.metrics.sequence_labeling import sequence_labeling_report
from coco_nlp.processors import SequenceProcessor
from coco_nlp.tasks.abs_task_model import ABCTaskModel
from coco_nlp.types import TextSamplesVar
...
class ABCLabelingModel(ABCTaskModel, ABC):
    ...
    def build_model(self,
        ...
        This function will setup a :class:`ABCGenerator`,
    ...
    def fit_generator(self,
                      train_sample_gen: ABCGenerator,
                      valid_sample_gen: ABCGenerator = None,
                      batch_size: int = 64,
                      epochs: int = 5,
                      callbacks: List['tf.keras.callbacks.Callback'] = None,
                      fit_kwargs: Dict = None) -> 'tf.keras.callbacks.History':
        ...
        for x, y in train_set.take(1):
            logger.debug('fit x input shape: {}'.format(np.array(x).shape))
            logger.debug('fit y input shape: {}'.format(np.array(y).shape))
    ...
    def predict(self,
        ...
        if predict_kwargs is None:
            predict_kwargs = {}
        with coco_nlp.utils.custom_object_scope():
            if truncating:
                seq_length = self.sequence_length
            else:
                seq_length = None
